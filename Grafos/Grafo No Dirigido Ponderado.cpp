/*
    ...
    Script: Como generar & guardar un grafo que sea de tipo no dirigido ponderado
    Lenguaje: C++
    Notas:
...
int main()
{
    ...
    for(int i=0; i<k; i++)
    {
        ...
        grafo[x].push_back(make_pair(y, p)); ///se coloca a y como vecino de x [x->y] con un peso de arista p
        grafo[y].push_back(make_pair(x, p)); ///se coloca a x como vecino de y [y->x] con un peso de arista p
    }
